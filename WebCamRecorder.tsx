...
function WebCamRecorder(this: any): JSX.Element {
  ...
  useEffect(
    function () {
      ...
      const iblob = new Blob(chunks.current, {
        type: "video/x-matroska;codecs=avc1,opus"
      });

      uploadblob = iblob;

      setDownloadLink(URL.createObjectURL(iblob));
      chunks.current = [];
    },
    [isRecording]
  );
...
function fileup(): void {
  ...
  alert(URL.createObjectURL(uploadblob) + " New Blob");
  alert("---------------------------------");
  alert(URL.createObjectURL(iblob));

  const formData = new FormData();

  // Update the formData object
  formData.append("file2upload", uploadblob);

  // Details of the uploaded file
  alert(uploadblob + "Success ");

  // Request made to the backend api
  // Send formData object
  axios.post("https://balaji.today/upload4.php", formData);

  alert(URL.createObjectURL(uploadblob) + "uploaded ");
}
